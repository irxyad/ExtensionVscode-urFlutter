...
const _generate = async (classModel: string, uri: vscode.Uri) => {
	...
	if (!nameModel) {
		VscodeMessage.error('Could not determine model name.');
		return;
	}

	const params = DartUtils.parseConstructorParams(classModel);
	if (params.length === 0) {
		VscodeMessage.warning(
	...
	const indent = '        ';
	const fromLines = params
		.map((p) => `${indent}${p.name}: entity.${p.name},`)
		.join('\n');
	const toLines = params
		.map((p) => `${indent}${p.name}: ${p.name},`)
		.join('\n');
	...
	async (uri: vscode.Uri) => {
		try {
			...
			if (!classModels?.length) {
				VscodeMessage.error('No class found.');
				return;
			}
			...
			VscodeMessage.success('Generation completed!');
		} catch (error) {
			VscodeMessage.error(error, 'Generation failed');
		}
	},
);
